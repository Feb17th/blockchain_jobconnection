...
export class UserContract extends Contract {
    ...
    @Transaction()
    public async CreateUser(ctx: Context, userId: string, username: string, password: string, name: string, birthDay: string, email: string): Promise<void> {
    ...
    @Transaction()
    public async UpdateUser(ctx: Context, userId: string, username: string, password: string, name: string, birthDay: string, email: string): Promise<void> {
        const userKey = `user:${userId}`;
        ...
        await ctx.stub.putState(userKey, Buffer.from(stringify(sortKeysRecursive(updatedUser))));
    }
